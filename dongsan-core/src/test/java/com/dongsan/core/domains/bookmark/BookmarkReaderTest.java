...
class BookmarkReaderTest {
	...
	class Describe_getUserBookmarkNames {
		...
		void 북마크가_존재하지_않으면_빈_리스트를_반환한다() {
			...
			Long memberId = 1L;
			Integer size = 10;
			when(bookmarkRepository.getUserBookmarks(size, lastCreatedAt, memberId)).thenReturn(
				Collections.emptyList());
	...
	class Describe_getBookmarksWithMarkedStatus {
		...
		void 북마크가_존재하면_북마크_추가여부를_리스트로_반환한다() {
			...
			when(bookmarkRepository.getBookmarksWithMarkedWalkway(walkwayId, memberId, createdAt, size)).thenReturn(
				bookmarks);

			// when
			List<BookmarkWithMarkedStatus> result = bookmarkReader.getBookmarksWithMarkedStatus(walkwayId, memberId,
				createdAt, size);
	...
	class Describe_existsMarkedWalkway {
		...
		void 빈_북마크_리스트를_전달하면_빈_map를_반환한다() {
			...
			when(bookmarkRepository.existsMarkedWalkway(walkwayId, emptyBookmarkIds)).thenReturn(
