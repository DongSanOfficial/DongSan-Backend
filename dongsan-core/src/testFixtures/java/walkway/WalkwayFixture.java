...
public class WalkwayFixture {
	...
	public static Walkway createWalkway() {
		return new Walkway(ID, NAME, CREATED_AT, MEMO, createStat(), HASHTAGS, createCourseInfo(),
			new Author(MEMBER_ID), ExposeLevel.PUBLIC);
	}

	public static Walkway createWalkwayPrivate() {
		return new Walkway(ID, NAME, CREATED_AT, MEMO, createStat(), HASHTAGS, createCourseInfo(),
			new Author(MEMBER_ID), ExposeLevel.PRIVATE);
	}

	public static Walkway createWalkwayWithId(Long walkwayId) {
		return new Walkway(walkwayId, NAME, CREATED_AT, MEMO, createStat(), HASHTAGS, createCourseInfo(),
			new Author(MEMBER_ID), ExposeLevel.PUBLIC);
	}
