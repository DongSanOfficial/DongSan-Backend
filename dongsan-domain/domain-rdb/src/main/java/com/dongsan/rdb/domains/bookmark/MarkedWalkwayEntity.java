...
public class MarkedWalkwayEntity extends BaseEntity {
	...
	public MarkedWalkway toMarkedWalkway() {
		return new MarkedWalkway(walkway.getId(), bookmark.getMember()
			.getId(),
			walkway.getName(), getCreatedAt(), walkway.getDistance(), walkway.getHashtags(),
			walkway.getCourseImageUrl(), walkway.getExposeLevel(), walkway.getLikeCount(),
			walkway.getReviewCount(), walkway.getRating());
	}
