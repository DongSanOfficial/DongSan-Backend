...
import com.dongsan.domain.refresh.GetTokenRemaining;
import com.dongsan.domain.refresh.TokenReader;
...
public class AuthService {
    ...
    public AuthService(TokenWriter tokenWriter, TokenReader tokenReader, JwtService jwtService,
     ...
     * 하라고 에러 반환 만약 refresh token 기간 만료 안됨 -> Id(Key) 확인 후 value 와 동일한 토큰 인지 확인 (갱신 전 토큰 일 수도) -> value 와 다른 토큰 : 다시
     * 로그인
    ...
    public GetTokenRemaining checkTokenExpire(HttpServletRequest request) {
        ...
        String refreshToken = cookieService.getRefreshTokenFromCookie(request);
        long accessTokenRemaining = accessToken == null ? 0 :
                jwtService.getAccessTokenRemainingTimeMillis(accessToken);
        long refreshTokenRemaining =
                refreshToken == null ? 0 : jwtService.getRefreshTokenRemainingTimeMillis(refreshToken);
        if (refreshTokenRemaining > 0) {
            Long memberId = jwtService.getMemberFromRefreshToken(refreshToken)
                    .getId();
