...
class BookmarkControllerTest {
    ...
    class Describe_getBookmarkDetailEntity {
        ...
        void 북마크에_추가된_산책로들의_정보를_response_DTO로_반환한다() throws Exception {
            ...
            List<MarkedWalkway> markedWalkways = new ArrayList<>(
                    List.of(createMarkedWalkway(1L, 3L), createMarkedWalkway(2L, 3L)));
            PagingResponse<MarkedWalkway> response = new PagingResponse<>(markedWalkways, false);
            when(bookmarkService.getBookmarkWalkways(customOAuth2User.getMemberId(), bookmarkId,
                    new CursorRequest(lastId, size))).thenReturn(response);
            CursorResponse<BookmarkWalkwaysResponse> result = new CursorResponse<>(
                    BookmarkWalkwaysResponse.from(response.data()), response.hasNext());
            ...
            mockMvc.perform(get("/bookmarks/{bookmarkId}/walkways", bookmarkId)
                            ...
                            .contentType("application/json;charset=UTF-8"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.size()", CoreMatchers.is(result.data()
                            .size())))
                    .andExpect(jsonPath("$.data[0].name").value(result.data()
                            .get(0)
                            .name()))
                    .andReturn();
        }
    ...
    class Describe_getUserBookmarks {
        ...
        void 북마크_이름을_response_DTO로_반환한다() throws Exception {
            ...
            when(bookmarkService.getUserBookmarksName(customOAuth2User.getMemberId(),
                    new CursorRequest(lastId, size))).thenReturn(response);
            CursorResponse<BookmarksNameResponse> result = new CursorResponse<>(
                    BookmarksNameResponse.from(response.data()), response.hasNext());
            ...
            mockMvc.perform(get("/users/bookmarks/title")
                            ...
                            .param("size", size.toString()))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.size()", CoreMatchers.is(response.data()
                            .size())))
                    .andExpect(jsonPath("$.data[0].title").value(response.data()
                            .get(0)
                            .title()))
                    .andExpect(jsonPath("$.data[1].title").value(response.data()
                            .get(1)
                            .title()));
        }
    }
