...
import static fixture.BookmarkEntityFixture.*;
import static fixture.MarkedWalkwayEntityFixture.*;
...
class BookmarkCoreRepositoryTest extends RepositoryTest {
	...
	class Describe_rename {
		...
		void Bookmark의_이름을_변경한다() {
			...
			assertThat(updateBookmark.get()
				.title()).isEqualTo(changedName);
		}
	}
	...
	class Describe_getUserBookmarks {
		...
		void it_returns_after_bookmarkId_bookmarks() {
			...
			// When
			List<Bookmark> result = bookmarkCoreRepository.getUserBookmarks(size, lastBookmarkEntity.getCreatedAt(),
				memberId);

			// Then
			assertThat(result)
				.hasSize(size)
				.isSortedAccordingTo(Comparator.comparing(Bookmark::createdAt)
					.reversed());
			assertThat(result)
				.allMatch(bookmark -> bookmark.createdAt()
					.isBefore(lastBookmarkEntity.getCreatedAt()));
			assertThat(result)
			...
			for (int i = 0; i < size; i++) {
				if (i < size - 1) {
					assertThat(result.get(i)
						.bookmarkId()).isGreaterThan(result.get(i + 1)
						.bookmarkId());
				}
			}
		}
	...
	class Describe_getBookmarksWithMarkedWalkway {
		...
		void it_returns_bookmarks() {
			...
			// When
			List<BookmarkWithMarkedStatus> result = bookmarkCoreRepository.getBookmarksWithMarkedWalkway(walkwayId,
				memberId, null, size);
