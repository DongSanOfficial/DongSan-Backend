...
class WalkwayCoreRepositoryTest {
	...
	class Describe_saveWalkway {
		...
		void it_returns_id() {
			...
			CreateWalkway createWalkway
				= new CreateWalkway("Sample", 2.5, 30, ExposeLevel.PUBLIC, point, point, "walkway.", lineString, null,
				null, memberId);
	...
	class Describe_saveLikedWalkway {
		...
		void it_returns_id() {
			...
			MemberEntity memberEntity = MemberEntityFixture.createMember();
			WalkwayEntity walkwayEntity = WalkwayEntityFixture.createWalkway(memberEntity);
			LikedWalkwayEntity likedWalkwayEntity = LikedWalkwayFixture.createLikedWalkway(memberEntity,
				walkwayEntity);
			when(memberJpaRepository.getReferenceById(memberId)).thenReturn(memberEntity);
	...
	class Describe_saveWalkwayHistoryTest {
		...
		void it_returns_id() {
			...
			MemberEntity memberEntity = MemberEntityFixture.createMember();
			WalkwayEntity walkwayEntity = WalkwayEntityFixture.createWalkway(memberEntity);
			WalkwayHistoryEntity historyEntity = WalkwayHistoryFixture.createWalkwayHistory(memberEntity,
				walkwayEntity);
	...
	class Describe_getCanReviewWalkwayHistory {
		...
		void it_returns_walkway_history() {
			...
			MemberEntity memberEntity = MemberEntityFixture.createMember();
			WalkwayEntity walkwayEntity = WalkwayEntityFixture.createWalkway(memberEntity);
			List<WalkwayHistoryEntity> walkwayHistoryEntities = List.of(
				WalkwayHistoryFixture.createWalkwayHistory(memberEntity, walkwayEntity));
			List<WalkwayHistory> walkwayHistories = walkwayHistoryEntities.stream()
				.map(WalkwayHistoryEntity::toWalkwayHistory)
				.toList();

			when(walkwayHistoryQueryDSLRepository.getCanReviewWalkwayHistories(walkwayId, memberId, size,
				null)).thenReturn(walkwayHistoryEntities);

			// when
			List<WalkwayHistory> result = walkwayCoreRepository.getCanReviewWalkwayHistory(walkwayId, memberId, size,
				null);
	...
	class Describe_getUserCanReviewWalkwayHistory {
		...
		void it_returns_walkway_history() {
			...
			List<WalkwayHistory> walkwayHistories = walkwayHistoryEntities.stream()
				.map(WalkwayHistoryEntity::toWalkwayHistory)
				.toList();
			when(walkwayHistoryQueryDSLRepository.getUserCanReviewWalkwayHistories(memberId, size,
				lastCreatedAt)).thenReturn(walkwayHistoryEntities);

			// when
			List<WalkwayHistory> result = walkwayCoreRepository.getUserCanReviewWalkwayHistory(memberId, size,
				lastCreatedAt);
